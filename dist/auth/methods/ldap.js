...
const ldap_service_ee_1 = require("../../ldap/ldap.service.ee");
const handleLdapLogin = async (loginId, password) => {
    if (!(0, helpers_ee_1.isLdapEnabled)())
        return undefined;
    const ldapService = typedi_1.Container.get(ldap_service_ee_1.LdapService);
    ...
    if (!ldapAuthIdentity) {
        ...
        if (emailUser && emailUser.email === emailAttributeValue) {
            ...
            typedi_1.Container.get(event_service_1.EventService).emit('user-signed-up', {
                user,
                userType: 'ldap',
