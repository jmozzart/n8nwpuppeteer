...
let SourceControlController = class SourceControlController {
    constructor(sourceControlService, sourceControlPreferencesService, eventService) {
    ...
    async getStatus(req) {
        try {
            const result = (await this.sourceControlService.getStatus(new source_control_get_status_1.SourceControlGetStatus(req.query)));
            return result;
        }
        catch (error) {
            throw new bad_request_error_1.BadRequestError(error.message);
    ...
    async status(req) {
        try {
            return await this.sourceControlService.getStatus(new source_control_get_status_1.SourceControlGetStatus(req.query));
        }
        catch (error) {
            throw new bad_request_error_1.BadRequestError(error.message);
        }
