...
let CacheService = class CacheService extends typed_emitter_1.TypedEmitter {
    ...
    async init() {
        ...
        if (useRedis) {
            ...
            const prefixBase = config_2.default.getEnv('redis.prefix');
    ...
    async expire(key, ttlMs) {
        ...
        if (this.cache.kind === 'memory') {
            throw new n8n_workflow_1.ApplicationError('Method `expire` not yet implemented for in-memory cache', {
                level: 'warning',
            });
        }
