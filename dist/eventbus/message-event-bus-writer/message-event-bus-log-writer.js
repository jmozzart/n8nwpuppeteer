...
class MessageEventBusLogWriter {
    constructor() {
        this.logger = typedi_1.default.get(logger_service_1.Logger);
    ...
    static async getInstance(options) {
        if (!MessageEventBusLogWriter.instance) {
            ...
            MessageEventBusLogWriter.options = {
                logFullBasePath: path_1.default.join(options?.logBasePath ?? typedi_1.default.get(n8n_core_1.InstanceSettings).n8nFolder, options?.logBaseName ?? typedi_1.default.get(config_1.GlobalConfig).eventBus.logWriter.logBaseName),
                keepNumberOfFiles: options?.keepNumberOfFiles ?? typedi_1.default.get(config_1.GlobalConfig).eventBus.logWriter.keepLogCount,
                maxFileSizeInKB: options?.maxFileSizeInKB ??
                    typedi_1.default.get(config_1.GlobalConfig).eventBus.logWriter.maxFileSizeInKB,
            };
