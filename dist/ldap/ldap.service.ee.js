...
let LdapService = class LdapService {
    ...
    async setGlobalLdapConfigVariables(ldapConfig) {
        ...
        config_1.default.set(constants_1.LDAP_LOGIN_LABEL, ldapConfig.loginLabel);
    }
    ...
    async getClient() {
        if (this.config === undefined) {
            throw new n8n_workflow_1.ApplicationError('Service cannot be used without setting the property config');
        }
    ...
    scheduleSync() {
        if (!this.config.synchronizationInterval) {
            throw new n8n_workflow_1.ApplicationError('Interval variable has to be defined');
        }
