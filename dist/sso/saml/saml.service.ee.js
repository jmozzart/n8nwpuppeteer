...
let SamlService = class SamlService {
    ...
    async init() {
        try {
            ...
            if (error instanceof invalid_saml_metadata_error_1.InvalidSamlMetadataError || error instanceof SyntaxError) {
    ...
    async loadSamlify() {
        ...
        this.samlify.setSchemaValidator({
            validate: async (response) => {
                const valid = await (0, saml_validator_1.validateResponse)(response);
                if (!valid) {
    ...
    async setSamlPreferences(prefs) {
        ...
        if (prefs.metadataUrl) {
            ...
            const validationResult = await (0, saml_validator_1.validateMetadata)(prefs.metadata);
            if (!validationResult) {
                throw new invalid_saml_metadata_error_1.InvalidSamlMetadataError();
            }
        }
    ...
    async loadPreferencesWithoutValidation(prefs) {
        ...
        await (0, saml_helpers_1.setSamlLoginEnabled)(prefs.loginEnabled ?? (0, saml_helpers_1.isSamlLoginEnabled)());
        (0, saml_helpers_1.setSamlLoginLabel)(prefs.loginLabel ?? (0, saml_helpers_1.getSamlLoginLabel)());
    }
    ...
    async fetchMetadataFromUrl() {
        ...
        try {
            ...
            if (response.status === 200 && response.data) {
                ...
                const validationResult = await (0, saml_validator_1.validateMetadata)(xml);
                if (!validationResult) {
    ...
    async reset() {
        ...
        await typedi_1.default.get(settings_repository_1.SettingsRepository).delete({ key: constants_1.SAML_PREFERENCES_DB_KEY });
    }
};
