...
let WorkerServer = class WorkerServer {
    constructor(globalConfig, logger, credentialsOverwrites, externalHooks, instanceSettings, prometheusMetricsService, redisClientService) {
    ...
    async mountEndpoints() {
        ...
        if (overwrites) {
            ...
            this.app.post(`/${endpoint}`, middlewares_1.rawBodyReader, middlewares_1.bodyParser, (req, res) => this.handleOverwrites(req, res));
        }
        if (metrics) {
    ...
    async readiness(_req, res) {
        const isReady = Db.connectionState.connected &&
