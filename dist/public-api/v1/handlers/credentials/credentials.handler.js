...
module.exports = {
    ...
    transferCredential: [
        ...
        async (req, res) => {
            ...
            await typedi_1.Container.get(credentials_service_ee_1.EnterpriseCredentialsService).transferOne(req.user, req.params.id, body.destinationProjectId);
            res.status(204).send();
        },
    ],
    deleteCredential: [
    ...
    getCredentialType: [
        async (req, res) => {
            ...
            try {
                typedi_1.Container.get(credential_types_1.CredentialTypes).getByName(credentialTypeName);
            }
            catch (error) {
                return res.status(404).json({ message: 'Not Found' });
            }
            const schema = typedi_1.Container.get(credentials_helper_1.CredentialsHelper)
