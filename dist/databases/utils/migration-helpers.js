...
const dsl_1 = require("../../databases/dsl");
const logger_service_1 = require("../../logging/logger.service");
const node_types_1 = require("../../node-types");
const PERSONALIZATION_SURVEY_FILENAME = 'personalizationSurvey.json';
function loadSurveyFromDisk() {
    try {
        const filename = `${typedi_1.Container.get(n8n_core_1.InstanceSettings).n8nFolder}/${PERSONALIZATION_SURVEY_FILENAME}`;
        ...
        if (!kvPairs.length) {
            throw new n8n_workflow_1.ApplicationError('personalizationSurvey is empty');
        }
        else {
            ...
            if (emptyKeys === kvPairs.length) {
                throw new n8n_workflow_1.ApplicationError('incomplete personalizationSurvey');
            }
        }
        return surveyFile;
    }
...
const runDisablingForeignKeys = async (migration, context, fn) => {
    ...
    if (dbType !== 'sqlite')
        throw new n8n_workflow_1.ApplicationError('Disabling transactions only available in sqlite');
...
const createContext = (queryRunner, migration) => ({
    logger: typedi_1.Container.get(logger_service_1.Logger),
    tablePrefix,
    dbType,
    isMysql,
    isSqlite,
    ...
    nodeTypes: typedi_1.Container.get(node_types_1.NodeTypes),
    loadSurveyFromDisk,
    parseJson,
    escape: {
...
const wrapMigration = (migration) => {
    ...
    if (up) {
        ...
        throw new n8n_workflow_1.ApplicationError(`Migration "${migration.name}" is missing the method \`up\`.`);
    }
    if (down) {
        Object.assign(migration.prototype, {
