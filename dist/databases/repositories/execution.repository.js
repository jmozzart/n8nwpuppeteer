...
const execution_annotation_ee_1 = require("../../databases/entities/execution-annotation.ee");
...
const execution_metadata_1 = require("../entities/execution-metadata");
...
function parseFiltersToQueryBuilder(qb, filters) {
    ...
    if (filters?.metadata) {
        qb.leftJoin(execution_metadata_1.ExecutionMetadata, 'md', 'md.executionId = execution.id');
...
let ExecutionRepository = class ExecutionRepository extends typeorm_1.Repository {
    ...
    reportInvalidExecutions(executions) {
        if (executions.length === 0)
            return;
        n8n_workflow_1.ErrorReporterProxy.error(new n8n_workflow_1.ApplicationError('Found executions without executionData', {
    ...
    async createNewExecution(execution) {
        ...
        if (dbType === 'sqlite' && sqliteConfig.poolSize === 0) {
            ...
            return await this.manager.transaction(async (transactionManager) => {
                const { identifiers: inserted } = await transactionManager.insert(execution_entity_1.ExecutionEntity, {
                    ...rest,
                    createdAt: new Date(),
                });
                const { id: executionId } = inserted[0];
    ...
    async softDeletePrunableExecutions() {
        ...
        const annotatedExecutionsSubQuery = this.manager
            ...
            .from(execution_annotation_ee_1.ExecutionAnnotation, 'annotation');
        const date = new Date();
    ...
    toQueryBuilder(query) {
        ...
        if (metadata?.length === 1) {
            ...
            qb.innerJoin(execution_metadata_1.ExecutionMetadata, 'md', 'md.executionId = execution.id AND md.key = :key AND md.value = :value');
        ...
        if (annotationTags?.length || vote) {
            ...
            if (annotationTags?.length) {
                for (let index = 0; index < annotationTags.length; index++) {
                    qb.innerJoin(annotation_tag_mapping_ee_1.AnnotationTagMapping, `atm_${index}`, `atm_${index}.annotationId = annotation.id AND atm_${index}.tagId = :tagId_${index}`);
        ...
        if (projectId) {
            qb.innerJoin(workflow_entity_1.WorkflowEntity, 'w', 'w.id = execution.workflowId')
                .innerJoin(shared_workflow_1.SharedWorkflow, 'sw', 'sw.workflowId = w.id')
    ...
    toQueryBuilderWithAnnotations(query) {
        ...
        return this.manager
            ...
            .leftJoin(annotation_tag_mapping_ee_1.AnnotationTagMapping, 'atm', 'atm.annotationId = e.annotation_id')
            .leftJoin(annotation_tag_entity_ee_1.AnnotationTagEntity, 'ate', 'ate.id = atm.tagId');
    }
    async getAllIds() {
