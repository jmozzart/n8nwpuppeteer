...
class ExportWorkflowsCommand extends base_command_1.BaseCommand {
    async run() {
        ...
        const workflows = await typedi_1.default.get(workflow_repository_1.WorkflowRepository).find({
            where: flags.id ? { id: flags.id } : {},
            relations: ['tags'],
        });
        if (workflows.length === 0) {
            throw new n8n_workflow_1.ApplicationError('No workflows found with specified filters');
        }
        if (flags.separate) {
            let fileContents;
            let i;
