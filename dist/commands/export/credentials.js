...
class ExportCredentialsCommand extends base_command_1.BaseCommand {
    async run() {
        ...
        const credentials = await typedi_1.default.get(credentials_repository_1.CredentialsRepository).findBy(flags.id ? { id: flags.id } : {});
        if (flags.decrypted) {
        ...
        if (credentials.length === 0) {
            throw new n8n_workflow_1.ApplicationError('No credentials found with specified filters');
        }
        if (flags.separate) {
            let fileContents;
            let i;
