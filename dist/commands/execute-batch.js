...
class ExecuteBatch extends base_command_1.BaseCommand {
    ...
    async stopProcess(skipExit = false) {
        ...
        await typedi_1.Container.get(active_executions_1.ActiveExecutions).shutdown(true);
        if (skipExit !== true) {
            process.exit(0);
        }
    }
    ...
    async run() {
        ...
        ExecuteBatch.instanceOwner = await typedi_1.Container.get(ownership_service_1.OwnershipService).getInstanceOwner();
        const query = typedi_1.Container.get(workflow_repository_1.WorkflowRepository).createQueryBuilder('workflows');
        if (ids.length > 0) {
                    ...
                    while (allWorkflows.length > 0) {
                        ...
                        await this.startThread(workflow).then((executionResult) => {
                            ...
                            if (executionResult.executionStatus === 'success') {
                                ...
                                throw new n8n_workflow_1.ApplicationError('Wrong execution status - cannot proceed');
                            }
                        });
                    }
                    resolve(true);
