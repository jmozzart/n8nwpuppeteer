...
class SecurityAudit extends base_command_1.BaseCommand {
    async run() {
        ...
        if (invalidCategories.length > 0) {
            ...
            throw new n8n_workflow_1.ApplicationError([message, hint].join('. '));
        }
        const result = await typedi_1.Container.get(security_audit_service_1.SecurityAuditService).run(categories, auditFlags['days-abandoned-workflow']);
...
SecurityAudit.flags = {
    ...
    'days-abandoned-workflow': core_1.Flags.integer({
        default: typedi_1.Container.get(config_1.SecurityConfig).daysAbandonedWorkflow,
