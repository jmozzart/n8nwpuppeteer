...
let License = class License {
    constructor(logger, instanceSettings, orchestrationService, settingsRepository, licenseMetricsService, globalConfig) {
        this.logger = logger;
    ...
    async onFeatureChange(_features) {
        ...
        if (config_2.default.getEnv('executions.mode') === 'queue' && this.globalConfig.multiMainSetup.enabled) {
            ...
            if (this.orchestrationService.isMultiMainSetupEnabled && this.instanceSettings.isFollower) {
                this.logger.debug('[Multi-main setup] Instance is follower, skipping sending of "reload-license" command...');
                return;
            }
        ...
        if (config_2.default.getEnv('executions.mode') === 'queue') {
            ...
            await typedi_1.default.get(Publisher).publishCommand({ command: 'reload-license' });
        }
