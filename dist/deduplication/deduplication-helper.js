...
const processed_data_repository_1 = require("../databases/repositories/processed-data.repository");
...
class DeduplicationHelper {
    ...
    static compareValues(mode, value1, value2) {
        if (mode === 'latestIncrementalKey') {
            ...
            throw new deduplication_error_1.DeduplicationError('Invalid value. Only numbers are supported in mode "latestIncrementalKey"');
        }
            ...
            try {
                ...
                throw new deduplication_error_1.DeduplicationError('Invalid value. Only valid dates are supported in mode "latestDate"');
            }
        }
        else {
            throw new deduplication_error_1.DeduplicationError("Invalid mode. Only 'latestIncrementalKey' and 'latestDate' are supported.");
        }
    }
    static createContext(scope, contextData) {
        if (scope === 'node') {
            if (!contextData.node) {
                throw new deduplication_error_1.DeduplicationError("No node information has been provided and so cannot use scope 'node'");
            }
    ...
    async findProcessedData(scope, contextData) {
        return await typedi_1.Container.get(processed_data_repository_1.ProcessedDataRepository).findOne({
            where: {
    ...
    validateMode(processedData, options) {
        if (processedData && processedData.value.mode !== options.mode) {
            throw new deduplication_error_1.DeduplicationError('Deduplication data was originally saved with an incompatible setting of the ‘Keep Items Where’ parameter. Try ’Clean Database’ operation to reset.');
        }
    }
    ...
    async handleLatestModes(items, contextData, options, processedData, dbContext) {
        ...
        if (!processedData) {
            await typedi_1.Container.get(processed_data_repository_1.ProcessedDataRepository).insert({
                workflowId: contextData.workflow.id,
                context: dbContext,
                value: {
                    mode: options.mode,
        ...
        await typedi_1.Container.get(processed_data_repository_1.ProcessedDataRepository).update({ workflowId: processedData.workflowId, context: processedData.context }, processedData);
        return returnData;
    }
    ...
    async removeProcessed(items, scope, contextData, options) {
        if (['latestIncrementalKey', 'latestDate'].includes(options.mode)) {
            throw new deduplication_error_1.DeduplicationError('Removing processed data is not possible in mode "latest"');
        }
        assert.ok(contextData.workflow.id);
        const processedData = await typedi_1.Container.get(processed_data_repository_1.ProcessedDataRepository).findOne({
            where: {
                workflowId: contextData.workflow.id,
                context: DeduplicationHelper.createContext(scope, contextData),
            },
    ...
    async clearAllProcessedItems(scope, contextData) {
        await typedi_1.Container.get(processed_data_repository_1.ProcessedDataRepository).delete({
            workflowId: contextData.workflow.id,
            context: DeduplicationHelper.createContext(scope, contextData),
        });
    }
    async getProcessedDataCount(scope, contextData, options) {
        const processedDataRepository = typedi_1.Container.get(processed_data_repository_1.ProcessedDataRepository);
